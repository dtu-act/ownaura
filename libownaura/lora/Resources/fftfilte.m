...
%   $Revision: #2 $  $Date: 2016/02/02 $ Axel Ahrens, aahr@elektro.dtu.dk
...
y = zeros(nx+nb-1,max(size(x,2),size(b,2))); %this is the difference to the original fftfilt (increased length of file)

% y = zeros(ceil(nx/L)*L+nb-1,max(size(x,2),size(b,2)));
istart = 1;
while istart <= nx
    ...
    Y = ifft(X.*B);
    yend = min(nx+nb-1,istart+nfft-1); %this is the difference to the original fftfilt (increased length of file)
